...
//	Return			:	true, if successful.
//
//	Purpose			:	Unloads all waves and pointers.
////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
int CSGD_WaveManager::LoadWave(const char* szFilename)
{
	...
	if (!pFile)
	{
		sprintf(szErrorBuffer, "Could not find the file - %s", szFilename);
		WAVEERRBOX(szErrorBuffer);
	}
	...
	{
		fclose(pFile);
		sprintf(szErrorBuffer, "Invalid Format Information - %s", szFilename);
		WAVEERRBOX(szErrorBuffer);
	}
	...
	{
		fclose(pFile);
		sprintf(szErrorBuffer, "Invalid PCM information - %s", szFilename);
		WAVEERRBOX(szErrorBuffer);
	}
	...
	{
		fclose(pFile);
		sprintf(szErrorBuffer, "Invalid Bits per Sample - Must be 8 or 16 - %s", szFilename);
		WAVEERRBOX(szErrorBuffer);
	}
	...
	{
		fclose(pFile);
		sprintf(szErrorBuffer, "Failed to make memory for wave information - %s", szFilename);
		WAVEERRBOX(szErrorBuffer);
	}
	...
	{
		delete [] pData;
		sprintf(szErrorBuffer, "Failed to Create the Sound Buffer - %s", szFilename);
		WAVEERRBOX(szErrorBuffer);
	}
...
//	Return			:	true, if successful.
//
//	Purpose			:	Releases a reference to a given wave. When the
//						reference to the wave is zero, the wave is
...
bool CSGD_WaveManager::Play(int nID, DWORD dwFlags)
{
	...
	// Make sure that the wave is valid
	assert(m_waveList[nID].dsBuffer != NULL && "Attempting to play released wave id");
	...
	else
	{
		...
		{
			char szErrorBuffer[256] = {0};
			sprintf(szErrorBuffer, "Failed to Play - %s", m_waveList[nID].filename);
			WAVEERRBOX(szErrorBuffer);
		}
	}
...
bool CSGD_WaveManager::Reset(int nID)
{
	...
	assert(nID > -1 && nID < (int)m_waveList.size() && "id is out of range");
	// Make sure that the wave is valid
	assert(m_waveList[nID].dsBuffer != NULL && "Wave buffer is NULL.  Possible unloaded wave.");
...
bool CSGD_WaveManager::SetFrequency(int nID, int nFreq)
{
	...
	if(m_dsObject->GetCaps(&caps)!= DS_OK)
	{
		WAVEERRBOX("Failed to get the device capabilities");
		return false;
	}

	if((unsigned)nFreq < caps.dwMinSecondarySampleRate)
		nFreq = caps.dwMinSecondarySampleRate;
	if((unsigned)nFreq > caps.dwMaxSecondarySampleRate)
		nFreq = caps.dwMaxSecondarySampleRate;
