...
CBattleMap::CBattleMap(char* szFileName, char* szMapName, int nNumEnemies)
{
	...
	m_pAnimation->Load("Resources/AnimationInfo/VG_Michaelangelo.dat");
	//m_pAnimation->Play();
	//CreateEnemies();
}
...
//		RECT srcRect = {0, 0, 64, 50};
	...
	{
		m_pTM->Draw(m_pFreeTiles[i].ImageID(), m_pFreeTiles[i].DestX()+(int)m_fScrollX, m_pFreeTiles[i].DestY()+(int)m_fScrollY,
