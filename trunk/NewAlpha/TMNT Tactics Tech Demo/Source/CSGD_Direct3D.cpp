...
bool CSGD_Direct3D::InitDirect3D(HWND hWnd, int nScreenWidth, int nScreenHeight, bool bIsWindowed, bool bVsync)
{
	...
	// set the view
	m_lpDirect3DDevice->SetRenderState(D3DRS_ZENABLE, true);
...
void CSGD_Direct3D::SetView()
{
	...
	// set view transform
	D3DXVECTOR3	eye(0,0,-10.00f);
	D3DXVECTOR3	lookat(0,0,0);
	D3DXVECTOR3	up(0,1,0);
	D3DXMatrixLookAtLH(&transform, &eye, &lookat, &up);
	m_lpDirect3DDevice->SetTransform(D3DTS_VIEW, &transform);
}

void CSGD_Direct3D::SetProjection()
{
	D3DXMATRIX transform;	// basic transform
	// field of view
	// aspect
	...
	D3DXMatrixPerspectiveFovLH(&transform, D3DX_PI/4, (float)1024/768, 0.0f, 1000.0f);
	m_lpDirect3DDevice->SetTransform(D3DTS_PROJECTION, &transform);
}
...
{
	m_lpDirect3DDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(ucRed, ucGreen, ucBlue), 100.0f, 0);
