...
namespace SGD
{
    public sealed class ManagedDirect3D
    {
        ...
        public bool DrawRectLine(Point topLeft, Point btmRight, Int32 red, Int32 green, Int32 blue)
        {
            if (line == null)
                return false;
            ...
            verts[0].X = topLeft.X;
            verts[0].Y = topLeft.Y;
            verts[1].X = btmRight.X;
            verts[1].Y = topLeft.Y;
            line.Draw(verts, System.Drawing.Color.FromArgb(red, green, blue));
            // bottom line
            verts[0].X = topLeft.X;
            verts[0].Y = btmRight.Y;
            verts[1].X = btmRight.X;
            verts[1].Y = btmRight.Y;
            line.Draw(verts, System.Drawing.Color.FromArgb(red, green, blue));
            // left line
            verts[0].X = topLeft.X;
            verts[0].Y = topLeft.Y;
